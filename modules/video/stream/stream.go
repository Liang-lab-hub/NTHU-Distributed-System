...
func (s *stream) HandleVideoCreated(ctx context.Context, req *pb.HandleVideoCreatedRequest) (*emptypb.Empty, error) {
	...
	if err := s.produceVideoCreatedWithScaleEvent(&pb.HandleVideoCreatedRequest{Id: req.GetId(), Url: req.GetUrl(), Scale: 1080,}); err != nil {
		return nil, &saramakit.HandlerError{Retry: true, Err: err}
	}

	if err := s.produceVideoCreatedWithScaleEvent(&pb.HandleVideoCreatedRequest{Id: req.GetId(), Url: req.GetUrl(), Scale: 720,}); err != nil {
		return nil, &saramakit.HandlerError{Retry: true, Err: err}
	}

	if err := s.produceVideoCreatedWithScaleEvent(&pb.HandleVideoCreatedRequest{Id: req.GetId(), Url: req.GetUrl(), Scale: 480,}); err != nil {
		return nil, &saramakit.HandlerError{Retry: true, Err: err}
	}

	if err := s.produceVideoCreatedWithScaleEvent(&pb.HandleVideoCreatedRequest{Id: req.GetId(), Url: req.GetUrl(), Scale: 320,}); err != nil {
		return nil, &saramakit.HandlerError{Retry: true, Err: err}
	}
